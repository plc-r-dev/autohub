...
export default async function ServiceStoreSetupServicesPage() {
  ...
  if (!progress || !branchId) {
    redirect("/service-store/setup/verify-business");
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <ServiceStoreCard className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <ServiceStoreButtonLink href={`/service-store/branches/${branchId}/services/new`}>
              Add service
            </ServiceStoreButtonLink>
            <Link href="/service-store/setup/hours" className="text-sm font-semibold text-[#0F9B76] hover:underline">
              Continue to hours
            </Link>
          </div>
        </ServiceStoreCard>
