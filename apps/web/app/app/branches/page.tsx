...
export default async function ServiceStoreBranchesPage({ searchParams }: PageProps) {
  ...
  return (
    <PageShell
      ...
      actions={<ServiceStoreButtonLink href="/app/branches/new">Add branch</ServiceStoreButtonLink>}
    >
          ...
          { key: "services", header: "Services", render: (branch) => branch._count.services },
          {
            ...
            render: (branch) => (
              <div className="flex flex-wrap gap-3 text-sm font-medium">
                <Link href={`/app/branches/${branch.id}`} className="text-[#0b7a3a] hover:underline">
                  Edit
                </Link>
                <Link href={`/app/branches/${branch.id}/hours`} className="text-[#0b7a3a] hover:underline">
                  Hours
                </Link>
                <Link href={`/app/branches/${branch.id}/services`} className="text-[#0b7a3a] hover:underline">
                  Services
                </Link>
              </div>
            ),
