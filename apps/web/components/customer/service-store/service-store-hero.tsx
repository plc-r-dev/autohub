...
import { ServiceStoreRatingBadge } from "@/components/customer/service-store/service-store-booking-card";
...
export function ServiceStoreHero({ serviceStoreId, name, description }: ServiceStoreHeroProps) {
  const [previewOpen, setPreviewOpen] = useState(false);
  const heroImage = getServiceShopImage(serviceStoreId, name, 0);
  const galleryImages = getServiceShopGalleryImages(serviceStoreId, name, 12);
