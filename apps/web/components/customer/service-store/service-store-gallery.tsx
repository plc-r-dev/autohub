...
export function ServiceStoreGallery({ serviceStoreId, serviceStoreName }: ServiceStoreGalleryProps) {
  const thumbnails = getServiceShopGalleryImages(serviceStoreId, serviceStoreName, 4);
  const allImages = getServiceShopGalleryImages(serviceStoreId, serviceStoreName, 12);
    ...
    <>
      <div className="grid grid-cols-4 gap-3">
        {thumbnails.slice(0, 3).map((image, index) => (
          <button
            key={`${serviceStoreId}-gallery-${index}`}
            type="button"
