...
export function MerchantCard({ merchant }: { merchant: MerchantCardData }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[20px] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.05)] transition-shadow hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
      ...
      <Link href={`/browse/${merchant.id}`} className="block">
        <div className="p-6">
          <h3 className="text-[18px] font-semibold tracking-tight text-[#0A0A0A]">{merchant.name}</h3>
          <p className="mt-1 text-[14px] text-[#64748B]">{merchant.distance ?? "1.2 km away"}</p>
          {merchant.startingPrice ? (
            <p className="mt-3 text-[15px] font-semibold text-[#0A0A0A]">
              From <span className="text-[#0F9B76]">{merchant.startingPrice}</span>
            </p>
          ) : null}
        </div>
      </Link>
      <div className="px-6 pb-6">
        {hasApprovedClaim ? (
          <ButtonLink href={`/browse/${merchant.id}`} size="md" className="w-full">
            Book Now
          </ButtonLink>
        ) : phone ? (
